...
class WebScraper:
    ...
    def runApp(self, BASE_URL, SITE_NAME):
        ...
        for link in categ_links:
            # The 2 lines below is basically saying: if the category link does not matches
            # https://discuss.pytorch.org/c/hackathon/16 then skip it. (Meaning the hackathon
            # category is the only category added to the list in line 153 and this code
            # will only run through the post in the hackathon category.)

            # if you want to run multiple categories in one run you just do 
            # if link.get_attribute('href') != 'https://discuss.pytorch.org/c/hackathon/16' and link.get_attribute('href') != 'https://discuss.pytorch.org/c/mac-os-x/32' and ...:

            if link.get_attribute('href') != 'https://discuss.pytorch.org/c/hackathon/16':
                continue
        ...
        for categ_url in categ_urls:
            ...
            # If you only want a certain number of post per category, you can just slice it like this:
            # This will loop over the first 500 post in the category
            # for categ_topic_url in categ_topic_urls[:500]:
            for categ_topic_url in categ_topic_urls:
                counter += 1 #to keep track of how many post it already ran through
                #Skipping over this one post due to some weird password requirement
                if categ_topic_url == 'https://discuss.pytorch.org/t/about-lr-range-test-y-onecyclescheduler/100671':
                    continue
...
if __name__=='__main__':
    ...
    webdriverPath = r'C:\Webdrivers\geckodriver' #CHANGE IT ACCORDING TO UR PATH
